...
const calculateBMIInPetList = (petList) => {
    if (petList) {
        ...
        for (let pet of petList) {
            if (pet.bmi === '?') {
                if (pet.type === 'Cat') {
                    bmi = (parseFloat(pet.weight) * 886) / parseFloat(pet.length) ** 2;
                    pet.bmi = bmi.toFixed(2);
                }

                else {
                    bmi = (parseFloat(pet.weight) * 703) / parseFloat(pet.length) ** 2;
                    pet.bmi = bmi.toFixed(2);
                }
            }
            newPetList.push(pet)
